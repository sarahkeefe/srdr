class BaselineCharacteristicFieldsController < ApplicationController
  ...
  def edit_custom
      @baseline_characteristic_field = BaselineCharacteristicField.find(params[:field_id])
	    respond_to do |format|
    	format.js{
    		render :update do |page|
    			page.replace_html 'population_characteristic_entry', :partial=>'baseline_characteristic_fields/edit_form'
    		end
  		}
  	end	
  end
...
respond_to do |format|
	...
	tmpl_id = @baseline_characteristic_field.template_id
	      format.js {
			  	render :update do |page|
						if params[:baseline_characteristic_field][:template_id].nil?
							#@baseline_characteristics_fields = BaselineCharacteristicField.where(:study_id => session[:study_id]).all
							...
							page['baseline_characteristic_field_form'].reset
  ...
  def destroy
    ...
    respond_to do |format|
      ...
      format.html { redirect_to(baseline_characteristic_fields_url) }
      format.xml  { head :ok }
    end
end
end
